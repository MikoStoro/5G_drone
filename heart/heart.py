...
db_path = 'drone.json'
open(db_path,'w') #truncate existing db
db = TinyDB(db_path)
'''
    EXAMPLE ENTRY:
    type: bt/zigbee
...
def shrink_db():
	size_diff = get_db_size() - settings["max_db_size"]
...
def push_entry(data, entry_type):
    ...
    size = get_db_size()
    if(size > settings["max_db_size"]):
		client.publish(heart_command_topic, "shrink_db")
    return entry_id
...
def on_message(client, userdata, msg):
	try:
		...
		if(len(msg.payload) > 1024): return
		...
		print("Error while processing message " + str(msg.payload))
